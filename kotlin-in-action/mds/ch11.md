...
### SQL DSL 만들기
```kotlin
// Table 내부에서 컬럼에 대한 기능을 확장하여 테이블에서만 사용할 수 있도록 한다.(이런걸 멤버 확장이라고 한다)
open class Table {
...
**코틀린은 다양한 기능으로 내부 DSL을 제공해줄 수 있으면서, 코틀린으로 DSL을 만들 때 가질 수 있는 강점은 정적 타입 언어이므로 내부 DSL을 만들어 사용할 때 문법 안전성을 보장해주며, 자동 완성까지 제공하는 것이다.**
