...
### 프록시의 두가지 목적
- 클라이언트가 타깃에 접근하는 방법을 제어하는 것(프록시 패턴)  
    - 프록시 패턴은 접근 제어뿐만 아니라 지연 초기화에 유용하다.
    - 클라이언트는 프록시를 참조하기 때문에 실제 사용시 싸지 객체 생성을 미룰 수 있다.
- 타깃에 부가적이 기능을 동적으로 부여해주는 것(데코레이터 패턴)
    - 프록시도 인터페이스로 타깃을 참조하므로 런타임에 실제 타깃이 정해진다.

> 프록시 패턴은 프록시의 일부분이다.


### 동적(다이나믹) 프록시
- 프록시를 직접 만들지 않고 리플렉션을 통해 런타임에 필요한 프록시를 생성하는 방법이다.
    - 프록시를 직접 만들면 프록시를 적용하지 않는 메서드는 하나하나 다 포워딩해줘야 하기 때문에 비효율적이다.
    - 그리고 모든 메서드에 동일한 프록시를 적용하고 싶을 때 쓸모 없는 중복이 생긴다.
    
#### 동적 프록시 InvocationHandler
- 동적 프록시는 InvocationHandler의 invoke를 이용하여 리플렉션을 통해 타겟 메서드를 호출할 수 있다.
- 이 invoke 메서드에서 프록시 부가 기능을 추가할 수 있다.
    - 보통 이걸 위해선 핸들러안에서 실제 대상이 될 target Object를 참조하고 있고 method.invoke할 때 target object를 이용하게 된다.
...
- 동적 프록시는 스프링 팩토리 빈을 활용하면 빈 등록시에도 유용하게 사용할 수 있다. (이건 config로 하는거 해보면 좋을거 같다.)

### 스프링의 프록시 팩토리 빈
- 스프링은 일관된 방법으로 프록시를 만들 수 있는 추상화 레이러를 제공한다.
- MethodInterceptor를 구현하여 ProxyFactoryBean에 addAdvice해주면된다. (addTarget으로는 실제 사용할 타겟을 넣자)
- MethodInterceptor는 메서드 파라미터로 타겟 메서드를 실행할 수 있는 콜백 정보(MethodInvoccation.proceed())를 제공하므로 타깃 오브젝마다 만들 필요가 없다.
    - 즉 싱글톤으로 만들어서 재사용 가능하다.
...
- 그러므로 싱글톤으로 만들어 재사용이 가능한 것이다. 
- 그리고 addAdvice로 어드바이스를 추가하기 때문에 여러개의 어드바이스를 추가할 수 있다.

### 포인트컷: 부가기능 적용 대상 메서드 선정 방법
- 프록시의 핵심은 부가기능이에 있기 때문에 부가기능을 적용할 대상은 분리하는게 좋다.
...
```
> 위와 같이 어드바이스와 포인트컷을 묶은 오브젝트를 어드바이저라고 하며 해당 어드바이저를 등록하여 원하는 메서드에만 부가 기능을 추가할 수 있다.
> - 포인트컷과 어드바이스는 각각 독립적이기 때문에 재사용이 가능하고, 이 둘을 결합안 어드바이저 또한 재사용이 가능하다.
...
![img](../images/beanpostprocessor.jpeg)
- 빈이 만들어 질때마다 빈 후처리기에서 어드바이저의 확인하여 타겟이 많다면 프록시 생성기를 통해 프록시를 생성하여 기존 빈과 연결한 후 기존 빈 오브젝트 대신 프록시를 컨테이너에게 돌려준다.
